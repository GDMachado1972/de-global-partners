...
.PHONY: help install lint test test-all infra-dryrun infra-apply silver-local \
        gold-local dashboard workflow-dryrun workflow-apply deploy reload clean
...
lint:            ## Syntax-check all Python (glue, infra, dashboard)
	$(PYTHON) -m compileall -q glue infra dashboard && echo "lint OK"
...
dashboard:       ## Run the Streamlit dashboard locally — requires LANDING=/path/to/csvs
	@test -n "$(LANDING)" || (echo "Set LANDING=/path/to/the/three/csvs"; exit 1)
	DATA_BACKEND=local LANDING=$(LANDING) $(PYTHON) -m streamlit run dashboard/app.py
